...
The **Sagadata Cloud Controller Manager** is an [out-of-tree](https://kubernetes.io/docs/tasks/administer-cluster/developing-cloud-controller-manager/#out-of-tree) Kubernetes Cloud Controller Manager for [EpiLayer](https://www.epilayer.eu/). It implements the standard [`cloudprovider.Interface`](https://github.com/kubernetes/cloud-provider/blob/master/cloud.go) from [k8s.io/cloud-provider](https://github.com/kubernetes/cloud-provider), so it can be used as the `--cloud-provider` backend for a Kubernetes control plane.

The CCM embeds cloud-specific control logic (or placeholders for it) and runs the shared controllers (Node, Route, Service) provided by Kubernetes. The current version is a **minimal implementation**: it registers the provider and runs the framework but does not yet implement Node, Route, or LoadBalancer behaviour. It is intended as a starting point to add real integration with the [EpiLayer API](https://developers.epilayer.eu/).

### 1.2 Relationship to other projects

| Project | Role |
|--------|------|
| **epilayer-cloud-controller-manager** (this repo) | Kubernetes integration: implements `cloudprovider.Interface` and runs as a CCM binary. |
| [terraform-provider-epilayer](https://github.com/epilayer-public/terraform-provider-epilayer) | Terraform integration: manages EpiLayer resources via Terraform. Same cloud, different interface. |
| [EpiLayer API](https://developers.epilayer.eu/) | Backend for instances, networking, etc. Can be used by both the CCM and the Terraform provider. |

This CCM does not depend on the Terraform provider. Future versions may share client code (e.g. [epilayer-go](https://github.com/epilayer-public/epilayer-go)) with the Terraform provider.

### 1.3 License

The project is licensed under the **Mozilla Public License Version 2.0** (MPL 2.0). See [LICENSE.txt](../LICENSE.txt) in the repository root. This aligns with the Terraform provider for EpiLayer.
...
2. Implement **cloudprovider.Interface** in our own package (`pkg/cloudprovider/epilayer`).
3. **Register** our provider in an `init()` so that `cloudprovider.InitCloudProvider("epilayer", configFile)` can instantiate it.
...
│  - Builds CCM options, default --cloud-provider=epilayer        │
...
│  - Looks up "epilayer" in the registry (set by our init())        │
│  - Calls our factory with config io.Reader → returns *cloud      │
└──────────────────────────────┬──────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│  pkg/cloudprovider/epilayer (*cloud)                             │
...
```
epilayer-cloud-controller-manager/
├── cmd/
...
| Path | Purpose |
|------|--------|
| `cmd/cloud-controller-manager/main.go` | Builds CCM options, sets default cloud provider to `epilayer`, defines `cloudInitializer` that calls `cloudprovider.InitCloudProvider`, and runs the CCM command. Imports the epilayer package so its `init()` runs and the provider is registered. |
| `pkg/cloudprovider/epilayer/cloud.go` | Defines `ProviderName`, the `cloud` struct, `newCloud(config)`, and `init()` that registers the provider. Implements all methods of `cloudprovider.Interface`; optional sub-interfaces return `(nil, false)`. |
| `pkg/cloudprovider/epilayer/cloud_test.go` | Tests that `newCloud(nil)` works, `ProviderName()` and `HasClusterID()` are correct, and that no optional interface is reported as supported. Includes `var _ cloudprovider.Interface = (*cloud)(nil)` for compile-time verification. |
...
The binary defaults `--cloud-provider` to **epilayer**, so you can omit that flag. You still need to pass at least a kubeconfig for a real cluster.
...
| Flag | Description |
|------|-------------|
| `--cloud-provider, default epilayer`. |
...
1. Our package’s `init()` has already registered the provider name "epilayer"`.
2. The app calls `cloudInitializer`, which calls `cloudprovider.InitCloudProvider("epilayer", cloudConfigFile)`.
...
The package `pkg/cloudprovider/epilayer` has a test file `cloud_test.go` that:

- Instantiates the provider with `newCloud(nil)`.
- Asserts `ProviderName() is "epilayer"` and `HasClusterID()` is `false`.
...
```bash
go test ./pkg/cloudprovider/epilayer/... -v
```
...
3. **Unit tests** — `go test ./pkg/cloudprovider/epilayer/...` passes (on a supported platform).
...
1. **InstancesV2** — Implement `InstancesV2()` to return a type that implements `InstanceExists`, `InstanceShutdown`, and `InstanceMetadata`. The node controller uses this to sync nodes with the cloud. You will need to call the EpiLayer API (or a shared client such as epilayer-go) to resolve node names or provider IDs to instance metadata.
...
3. **LoadBalancer** — To support `LoadBalancer` Services, implement `LoadBalancer()` and the Get/Ensure/Update/Delete methods; integrate with EpiLayer load balancer or networking APIs.
...
- [EpiLayer](https://www.epilayer.eu/) — [API documentation](https://developers.epilayer.eu/)
- [Terraform provider for EpiLayer](https://github.com/epilayer-public/terraform-provider-epilayer) — same cloud, Terraform interface; MPL 2.0
