...
const (
	...
	AnnotationLoadBalancerName = "epilayer.io/loadbalancer-name"
...
func (lb *loadBalancers) floatingIPByName(ctx context.Context, name string) (*epilayer.FloatingIP, error) {
	page := 1
	for {
		resp, err := lb.client.ListFloatingIPsPaginatedWithResponse(ctx, &epilayer.ListFloatingIPsPaginatedParams{
			Page: &page,
		})
		if err != nil {
...
func (lb *loadBalancers) ensureFloatingIP(ctx context.Context, svc *v1.Service) (string, error) {
	...
	if fip == nil {
		...
		resp, err := lb.client.CreateFloatingIPWithResponse(ctx, epilayer.CreateFloatingIPJSONRequestBody{
			Name:   name,
			Region: lb.region,
		})
		if err != nil {
	...
	for fip.Status != epilayer.FloatingIpStatusCreated {
		if fip.Status == epilayer.FloatingIpStatusError {
